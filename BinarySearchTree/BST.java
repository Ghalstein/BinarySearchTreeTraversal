...
public class BST<Key extends Comparable<Key>, Value> {
    ...
    // returns the keys in order from gretest to least
    public Iterable<Key> keyOrder() {
        return keyRangeSearch(min(), max());
    }
    ...
    public Iterable<Key> keyRangeSearch(Key lo, Key hi) {
        if (lo == null) throw new IllegalArgumentException("first argument to keys() is null");
        if (hi == null) throw new IllegalArgumentException("second argument to keys() is null");
 
        Stack<Key> stack = new Stack<Key>();
        keyOrder(root, stack, lo, hi);
        return stack;
    }
    ...
    private void keyOrder(Node x, Stack<Key> stack, Key lo, Key hi){
        if (x == null) return;
        int cmplo = lo.compareTo(x.key);
        int cmphi = hi.compareTo(x.key);
        if (cmplo < 0) keyOrder(x.left, stack, lo, hi);
        if (cmplo <= 0 && cmphi >= 0) stack.push(x.key);
        if (cmphi > 0) keyOrder(x.right, stack, lo, hi);
    }
